...
#define STRING_SHOW_EXIT_SUB_FUNCTION "[exit:%d] "
#define STRING_SHOW_SIGN_SUB_FUNCTION "[sign:%d] "
...
void execute_sub_command(char* strInputShell, char* statusBuffer)
{
    ...
    else
    {
        ...
        if (WIFEXITED(status))
        {
            ...
            snprintf(statusBuffer, MAX_SIZE, STRING_SHOW_EXIT_SUB_FUNCTION, exitCode);
        }
        else if (WIFSIGNALED(status))
        {
            int signalNumber = WTERMSIG(status);
            snprintf(statusBuffer, MAX_SIZE, STRING_SHOW_SIGN_SUB_FUNCTION, signalNumber);
        }
    }
}
